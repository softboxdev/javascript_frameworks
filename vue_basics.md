...
Теперь вы готовы создавать сложные Vue приложения! 🚀
...
<template>
  <div>
    <!-- user-profile - использование дочернего компонента -->
    <!-- :user - привязка prop (передача данных вниз) -->
    <!-- :age - привязка числа (без кавычек) -->
    <!-- @update-user - прослушивание события (получение данных вверх) -->
    <user-profile 
      :user="currentUser"
      :age="30"
      :is-admin="true"
...
<script>
// import - импорт компонента из файла
import UserProfile from './UserProfile.vue'

export default {
  // components - регистрация локальных компонентов
  components: {
    // UserProfile - имя компонента для использования в шаблоне
    UserProfile
  },
  data() {
    return {
      // currentUser - данные для передачи в дочерний компонент
      currentUser: {
        name: 'Анна',
  ...
  methods: {
    // handleUserUpdate - обработчик события из дочернего компонента
    handleUserUpdate(updatedUser) {
      // updatedUser - данные, переданные из дочернего компонента
...
<template>
  <div class="user-profile">
    <!-- {{ user.name }} - вывод prop -->
    <h3>{{ user.name }}</h3>
    
    <!-- {{ age }} - вывод другого prop -->
    <p>Возраст: {{ age }}</p>
    
    <!-- v-if - условный рендеринг по prop -->
    <span v-if="isAdmin" class="admin-badge">Администратор</span>
    
    <!-- @click - обработчик клика -->
    <!-- updateUser - метод компонента -->
...
export default {
  ...
  // props - объявление входящих параметров
  props: {
    // user - prop типа Object
    user: {
      type: Object,
      // required: true - обязательный параметр
      required: true,
      // default - значение по умолчанию
      default: () => ({ name: 'Гость', email: '' })
    },
    
    // age - prop типа Number
    age: {
      type: Number,
  ...
  emits: {
    // 'update-user' - событие без валидации
    'update-user': null,
    
    // С событием с валидацией
    'delete-user': (payload) => {
      // payload - данные события
      // Валидация: вернуть true/false
  ...
  methods: {
    updateUser() {
      ...
      // $emit - метод для генерации пользовательского события
      // 'update-user' - имя события
      // updatedUser - данные (payload)
